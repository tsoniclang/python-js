...
| Objects | `JsObject`, `property_key`, `get_property`, `set_property`, `delete_property` | implemented | C# `ObjectTests.cs`, Rust `object_tests.rs` behavior subset |
| Dynamic values | `JsValue` | implemented for closed compat lanes | Rust `JsValue` equality/property expectations |
| Arrays | `JsArray` methods | implemented | C# `ArrayTests.cs`, Rust `js_array_sparse_tests.rs` |
| Strings | UTF-16 and common non-RegExp helpers | implemented | C# `StringTests.cs`, Rust `string_tests.rs` |
